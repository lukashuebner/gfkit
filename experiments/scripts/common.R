...
# Generate a named vector to use as a labeller
labeller_factory <- function(labels, pretty_printer) {
...
theme_husky <- function(style, ..., text_size = waiver()) {
    husky <- 
        theme_bw() +
        theme(
...
# Create a labeller for log-axis

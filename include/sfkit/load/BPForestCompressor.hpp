...
public:
    BPForestCompressor(TSKitTreeSequence& tree_sequence) : _ts_tree(tree_sequence), _is_reference() {
        ...
        for (_ts_tree.first(); _ts_tree.is_valid(); _ts_tree.next()) {
            ...
            while (node_it != eulertour.end()) {
                ...
                if (node_it.is_sample()) {
                    ...
                    std::cout << "ts_node_id " << ts_node_id << " | sample\n";
                } else if (node_it.first_visit()) {
                    // The subtree that begins here (i.e. children not yet processed, has not been added to the BP,
                    // add it.
                    _open_subtree();
                    std::cout << "ts_node_id " << ts_node_id << " | open subtree\n";
                } else {
                    KASSERT(node_it.second_visit());
                    // Compute the subtree ID of this inner node by hashing the subtree IDs of its children.
                    // TODO Build an abstraction around this
                    ...
                    // TODO Use a stack to save the subtree IDs? Also in the other compression algorithm.
                    ...
                    if (reference_it != _subtrees.end()) {
                        ...
                        _rollback_subtree();
                        _refer_to(reference_it);
                        std::cout << "ts_node_id " << ts_node_id << " | rollback and refer subtree\n";
                    } else {
                        ...
                        std::cout << "ts_node_id " << ts_node_id << " | close and commit subtree\n";
                    }
                }
                ++node_it;
            }
...
private:
    ...
    static constexpr bool PARENS_OPEN  = true;
    static constexpr bool PARENS_CLOSE = false;
    ...
    DynamicSDSLContainer<sdsl::bit_vector>     _is_reference;
    DynamicSDSLContainer<sdsl::bit_vector>     _balanced_parenthesis;
    DynamicSDSLContainer<sdsl::int_vector<> >  _references;
    DynamicSDSLContainer<sdsl::int_vector<> >  _leaves;
    std::vector<SubtreeStart>                  _subtree_starts;
    tsl::hopscotch_map<SubtreeHash, Reference> _subtrees;
    ...
    void _add_sample(SampleId const sample_id) {
        ...
        // _is_reference.push_back(false);
        // _balanced_parenthesis.push_back(PARENS_OPEN);
        // _balanced_parenthesis.push_back(PARENS_CLOSE);
        // The subtree id is already registered
        // TODO change this
        _open_subtree();

        // Compute the subtree ID of this sample (leaf) node by hashing its label.
        auto subtree_id = _subtree_hash_factory.compute(sample_id);

        // Cache the subtree ID for this TS node
        KASSERT(asserting_cast<size_t>(sample_id) < _ts_node_to_subtree.size());
        _ts_node_to_subtree[asserting_cast<size_t>(sample_id)] = subtree_id;
        _close_and_commit_subtree(subtree_id);
        ++_num_samples;
    }
    ...
    void _open_subtree() {
        ...
        _subtree_starts.emplace_back(_balanced_parenthesis.index(), _references.index());
        std::cout << "subtree_starts.size() " << _subtree_starts.size() << "\n";
        std::cout << "_balanced_parenthesis.index() " << _balanced_parenthesis.index() << "\n";
        std::cout << "_references.index() " << _references.index() << std::endl;
        _is_reference.push_back(false);
        _balanced_parenthesis.push_back(PARENS_OPEN);
    }

    void _close_and_commit_subtree(SubtreeHash subtree_id) {
        _is_reference.push_back(false);
        _balanced_parenthesis.push_back(PARENS_CLOSE);
